...
void ILI9488Driver::drawStringScaled(uint16_t x, uint16_t y, const char* str, uint32_t color, uint32_t bg_color, unsigned scale) {
    drawStringScaled(x, y, std::string_view(str), color, bg_color, scale);
}

void ILI9488Driver::drawStringScaled(uint16_t x, uint16_t y, std::string_view str, uint32_t color, uint32_t bg_color,
                                     unsigned scale) {
    using namespace font;
    if (scale < 1u) {
        scale = 1u;
    ...
    const uint16_t dw = getWidth();
    const uint16_t dh = getHeight();
    const uint16_t fg565 = ili9488_colors::rgb888_to_rgb565(color);
    const uint16_t bg565 = ili9488_colors::rgb888_to_rgb565(bg_color);
    ...
    for (char c : str) {
        ...
        for (uint8_t row = 0; row < FONT_HEIGHT; ++row) {
            uint8_t byte = char_data[row];
            for (uint8_t col = 0; col < FONT_WIDTH; ++col) {
                const bool pixel_is_set = ((byte >> (7 - col)) & 0x01) != 0;
                const uint16_t px565 = pixel_is_set ? fg565 : bg565;
                uint16_t x0 = static_cast<uint16_t>(cx + col * scale);
                uint16_t y0 = static_cast<uint16_t>(y + row * scale);
                uint16_t x1 = static_cast<uint16_t>(x0 + scale - 1);
                uint16_t y1 = static_cast<uint16_t>(y0 + scale - 1);
                if (x0 >= dw || y0 >= dh) {
                    continue;
                }
                if (x1 >= dw) {
                    x1 = static_cast<uint16_t>(dw - 1);
                }
                if (y1 >= dh) {
                    y1 = static_cast<uint16_t>(dh - 1);
                }
                fillArea(x0, y0, x1, y1, px565);
            }
        }

        cx = static_cast<uint16_t>(cx + FONT_WIDTH * scale);
        if (cx >= dw) {
            break;
        }
    }
